package kattcrazy.sharemything.tile
...
import kattcrazy.sharemything.theme.toProtolayoutColorScheme
...
private fun MaterialScope.bothTileLayout(
    ...
    .setHorizontalAlignment(LayoutElementBuilders.HORIZONTAL_ALIGN_CENTER)
    .setModifiers(openItemModifiers(context, item))
    .addContent(qrWhiteSection(deviceConfiguration, item, SurfaceDisplayConstants.BOTH_QR_WIDTH_FRACTION))
    .addContent(titleSection(item.title))
    .addContent(
        LayoutElementBuilders.Column.Builder()
            .setWidth(DimensionBuilders.expand())
                ...
                ModifiersBuilders.Modifiers.Builder()
                    .setPadding(
                        ModifiersBuilders.Padding.Builder()
                            .setTop(DimensionBuilders.dp(SurfaceDisplayConstants.TEXT_CONTENT_TOP_PADDING_DP))
                            .setStart(DimensionBuilders.dp(SurfaceDisplayConstants.TILE_TEXT_HORIZONTAL_PADDING_DP))
                            .setEnd(DimensionBuilders.dp(SurfaceDisplayConstants.TILE_TEXT_HORIZONTAL_PADDING_DP))
                            .build(),
                    )
                    .build(),
            )
            .addContent(
                tileBodyText(
                    value = item.content,
                    maxLines = SurfaceDisplayConstants.bothTileContentMaxLines(deviceConfiguration.screenHeightDp.toFloat()),
                ),
            )
            .build(),
    )
    ...
    .addContent(
        LayoutElementBuilders.Column.Builder()
            ...
            .addContent(
                tileBodyText(
                    value = item.content,
                    maxLines = SurfaceDisplayConstants.textContentMaxLines(deviceConfiguration.screenHeightDp.toFloat()),
                ),
            )
            .build(),
    )
...
private fun MaterialScope.qrWhiteSection(
    deviceConfiguration: DeviceParametersBuilders.DeviceParameters,
    item: DisplayItem,
    widthFraction: Float = SurfaceDisplayConstants.QR_IN_APP_WIDTH_FRACTION,
): LayoutElementBuilders.Column {
    val qrSizeDp = deviceConfiguration.screenWidthDp * widthFraction
    val imageSizeDp = (qrSizeDp - SurfaceDisplayConstants.QR_IMAGE_INNER_PADDING_DP * 2f).coerceAtLeast(48f)
    val columnBuilder = LayoutElementBuilders.Column.Builder()
        ...
        .setModifiers(
            ModifiersBuilders.Modifiers.Builder()
                ...
                .setPadding(
                    ModifiersBuilders.Padding.Builder()
                        .setTop(DimensionBuilders.dp(SurfaceDisplayConstants.QR_WHITE_TOP_PADDING_DP))
                        .setStart(DimensionBuilders.dp(SurfaceDisplayConstants.QR_WHITE_HORIZONTAL_PADDING_DP))
                        .setEnd(DimensionBuilders.dp(SurfaceDisplayConstants.QR_WHITE_HORIZONTAL_PADDING_DP))
                        .setBottom(DimensionBuilders.dp(SurfaceDisplayConstants.QR_WHITE_BOTTOM_PADDING_DP))
                        .build(),
                )
                .build(),
        )
    ...
    if (item.type.usesQr && item.content.isNotBlank()) {
        val imageResource = runCatching {
            QrCodeGenerator.generate(item.content, SurfaceDisplayConstants.QR_TILE_SIZE_PX)?.toInlineImageResource()
        }.getOrNull()
    ...
    LayoutElementBuilders.Column.Builder()
        ...
        .setModifiers(
            ModifiersBuilders.Modifiers.Builder()
                .setPadding(
                    ModifiersBuilders.Padding.Builder()
                        .setTop(DimensionBuilders.dp(SurfaceDisplayConstants.TITLE_TOP_PADDING_DP))
                        .setStart(DimensionBuilders.dp(SurfaceDisplayConstants.TILE_TEXT_HORIZONTAL_PADDING_DP))
                        .setEnd(DimensionBuilders.dp(SurfaceDisplayConstants.TILE_TEXT_HORIZONTAL_PADDING_DP))
                        .build(),
                )
