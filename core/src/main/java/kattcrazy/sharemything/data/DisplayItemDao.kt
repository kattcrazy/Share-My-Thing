...
interface DisplayItemDao {
    ...
    @Transaction
    suspend fun reorderItems(orderedIds: List<Long>, updatedAtMillis: Long) {
        orderedIds.forEachIndexed { index, id ->
            val item = getById(id) ?: return@forEachIndexed
            if (item.sortOrder != index) {
                update(item.copy(sortOrder = index, updatedAtMillis = updatedAtMillis))
            }
        }
    }
