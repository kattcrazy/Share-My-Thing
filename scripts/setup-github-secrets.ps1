...
<#
.SYNOPSIS
  Upload Share My Thing signing credentials to GitHub Actions secrets (one-time setup).
...
try {
    ...
    $base64File = Join-Path $env:TEMP "sharemything-keystore.b64"
    [IO.File]::WriteAllText($base64File, $base64, (New-Object System.Text.UTF8Encoding $false))

    Write-Host "Setting GitHub secrets on kattcrazy/Share-My-Thing..." -ForegroundColor Cyan
    ...
    Write-Host ""
    Write-Host "Done. GitHub Actions can now sign releases with your Play Console upload key." -ForegroundColor Green
    Write-Host "Upload key SHA-256: FF:05:C1:1D:4F:EE:B2:A7:52:9F:48:48:1D:41:D1:6C:25:71:EB:DC:17:98:F3:DB:BD:9B:F8:E4:3C:1F:86:C7"
